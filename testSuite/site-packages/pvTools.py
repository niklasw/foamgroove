...
class Field:
    ...
    def initFromSetup(self,setup):
        ...
        print('Using function {0} with data mode {1}'.format(self.function, self.displayMode))
...
class PvTools:
    ...
    def createVectorRange(startV=(0,0,0), endV=(1,1,1), N=5):
        ...
        startV = list(map(float,startV))
        endV = list(map(float,endV))
    ...
    def __init__(self,dataFile='case.foam', fieldName=None):
        ...
        print(self.foamCase)
        self.currentSource = ExtractBlock(Input=self.foamCase)
        self.currentSource.Selectors = ['//internalmesh']
        self.views = []
        self.currentView = -1 
    ...
    def initFromSetup(self,setup):
        self.foamCase.CaseType = setup.CASE_TYPE
        self.foamCase.UpdatePipelineInformation()
    ...
    def loadCase(self):
        source = OpenFOAMReader(FileName=self.dataFile)
        SetActiveSource(source)
        return source

    def loadCase_old(self):
        OpenDataFile(self.dataFile)
        sourcesDict = GetSources()
        source = list(sourcesDict.values())[0]
        SetActiveSource(source)
        return source
    ...
    def updatePipeLine(self, newSource):
        ...
        SetActiveSource(self.currentSource)

    def initDisplayAndTime(self,bg=[0.2,0.2,0.2,],viewSize=[2000,1000], \
                           viewName='RenderView'):
        self.foamCase.UpdatePipelineInformation()
        ...
        self.instants = self.foamCase.TimestepValues
        try:
            self.setLatestTime()
    ...
    def hideDefaultRepresentation(self):
        rep = GetDisplayProperties(self.foamCase)
        rep.Visibility = 0
    ...
    def calculator(self):
        Hide()
        calc = Calculator(Input=self.currentSource)
        calc.AttributeType = self.field.functionMode
...
if __name__=="__main__":
    ...
    class setup:
        ...
        SCALAR_FIELD    = 'k'
        FIELD_FUNCTION  = 'sqrt(k)*sqrt(mag(U))'
        SCALAR_MIN      = 0
        SCALAR_MAX      = 0.1
        NORMAL          = (0,0,1)
        ORIGIN          = (0,0,0.005)
        CAMERA_UP       = (0,1,0)
        CAMERA_POS      = (0.05, 0.05, 0.3)
        CAMERA_FOCUS    = (0.05, 0.05, 0.005)
        CAMERA_ZOOM     = 16
        IMAGE_SCALE     = 1
        VIEW_SIZE       = [500,500]
        LEGEND          = True
