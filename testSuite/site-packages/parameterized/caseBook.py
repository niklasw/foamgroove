...
from .suiteUtils import Paths,Info,Error,Debug
        ...
        if not self._assert():
            ...
            for row in self.data:
                ...
                if not headerDone:
                    headerDone = True
                    while len(row) < maxL:
                        self.columnNames.append(next(letters))
...
class Book:
    ...
    def open(root=None):
        ...
        if os.path.isfile(dbPath):
            Debug('Opening Book at db path = '+dbPath)
    ...
    def parameterString(self,separator='; '):
        return separator.join(map(str,list(self.parameters.values())))
    ...
    def summaryDict(self):
        ...
        #    summary[param] = value
        if 'run' not in self.timer:
            self.timer['run'] = -1
        summary['time'] = self.timer['run']
        return summary

    def summaryString(self,separator='; '):
        return separator.join( \
                    ['{0}:={1}'.format(a,b) \
                    for a,b in self.summaryDict().items()])
    ...
    def printHtml(self):
        ...
        params =htmlSection('Paramters',self.parameterString('<br/>'),level=2)

        procTimes = []
        for proc in list(self.timer.keys()):
