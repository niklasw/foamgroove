...
import os,re
from .suiteUtils import Info,Error,Debug,Warning,findDirs,Paths,pathTail
from . import htmlTemplates

class htmlTemplate:
    def __init__(self, templateStr, root='',relRoot=''):
        self.root = root
...
class htmlTable:
    ...
    def __init__(self, rowList, head=[], cls='',ID='',description=''):
        self.rows = rowList
        self.head = head
        self.cls = cls
        self.id = ID
    ...
    def generate(self):
        ...
        start = '<table class="{0}" id="{1}">\n'.format(self.cls,self.id)
        end   = '\n</table>'
...
class htmlLink:
    def __init__(self,name='Press here',href='#',cls='',relative=False):
        if relative:
            print('------------------------')
            print(href)
            href=os.path.relpath(href,Paths().PresentRoot)
            print(href)
...
class htmlImage:
    def __init__(self,name,src,cls='',description='',relative=False):
        self.name = name
        if relative:
