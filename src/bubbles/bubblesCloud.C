...
Foam::bubblesCloud::bubblesCloud
(
    const fvMesh& mesh,
...
:
    Cloud<bubbles>(mesh, cloudName, false),
    mesh_(mesh),
    particleProperties_
    (
        IOobject
...
bool Foam::bubblesCloud::hasWallImpactDistance() const
{
    return true;
}


void Foam::bubblesCloud::move(const dimensionedVector& g)
{
    ...
    bubbles::trackingData
        td(*this, rhoInterp, UInterp, nuInterp, g.value());

    Cloud<bubbles>::move(td, mesh_.time().deltaTValue());
}
