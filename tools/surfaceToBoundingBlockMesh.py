...
class BoundingBlockDict:
    ...
    def interactor(self):
        ...
        self.offset = i.get(prompt='Offset factor', default=0.0,test=float)
        self.infoRefinementLevels(self.size,1,8)
        self.output = i.get(test=str,default='system/blockMeshDict')
    ...
    def selectSurfaceTool(self):
        import os
        suffix = os.path.splitext(self.fileName)[1]
        if suffix == '.stl':
            from surfaceTool import objToolBox as toolbox
            return toolbox
        elif suffix == '.obj':
            from surfaceTool import objToolBox as toolbox
            return toolbox
    ...
    def createBlock(self, name=''):
        ...
        toolbox = self.selectSurfaceTool()
        tool = toolbox(self.fileName,'',False)

        min,max = tool.calcBounds()
        max = asarray(max)
        min = asarray(min)
        # Extend the bounding box by offset in each direction
        max += self.offset
        min -= self.offset
        diagonal = max - min
