...
class ioList(list):
    ...
    def __init__(self,var='(0 0 0)'):
        if isinstance(var,list):
            ...
            s = strList.strip('() "')
            try:
                list.__init__(self,list(map(float, s.split())))
            except TypeError:
...
class interactor:
    ...
    def iFileSelector(self,prompt='Select from above',default='',path='',suffix='*',folder=False, stripShow=False):
        ...
        isel = self.get(prompt=prompt,default=0,test=int,allowed=list(range(len(selectables))))
    ...
    def fileSelector(self,prompt='Select from above',default='',path='',suffix='*',folder=False):
        ...
        try:
            selected = input('%s [%s]: '% (prompt,selectables[0])).strip()
    ...
    def get(self,prompt='Enter value',default=10,test=int, allowed=[], showAllowed=False):
        ...
        if showAllowed:
            for ok in allowed:
                self.info('|--> %s '%ok)
        try:
            selected = input('%s [%s]: '% (prompt,default))
            if not selected:
                selected = default
    ...
    def yesno(self,prompt='Enter value',default='N',test=str,allowed=['y','n','Y','N']):
        print()
        try:
            selected = input('%s (y/n) [%s]: '% (prompt,default))
