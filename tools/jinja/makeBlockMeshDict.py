...
from blockMesh import Blocks,Block2D,strList,template
...
def run():
    ...
    b4.addBoundary('x1', 'ambience')
    b4.addBoundary('y1', 'ambience')
    b4.addBoundary('z0', 'wedge0')
    b4.addBoundary('z1', 'wedge1')
    
    B = Blocks([b0,b1,b2,b3,b4])
    B.makePatch('ambience','patch')
    B.makePatch('walls','wall')
    B.makePatch('inlet','patch')
    #B.makePatch('outlet','patch')
